...
components:
  ...
  caesura:
    ...
    padding: '16px 16px'
  nav-movement:
    backgroundColor: 'accent tint at 9% on hover'
    textColor: 'movement accent mixed with {colors.ink-secondary}'
    typography: '{typography.readout}'
    rounded: '{rounded.none}'
    padding: '6px 10px'
  nav-movement-active:
    textColor: 'movement accent'
---
...
Density is high and unapologetic. The reading is horizontal: stage barlines cross every voice left to right, task voices are placed by dependency depth, and ties bind a dependent to what it waited for. The one place the system stops is the caesura — a full-width vermilion hold carrying the control that releases it. Colour is load-bearing at two levels: semantic inks identify state, while a broader annotation family assigns stable hues to navigation, section bands, categories, and interactive surfaces. This lets the hierarchy register before the text is read without turning the score into a dashboard of filled cards.
...
- Diazo blueprint ground (`#101a33`) with engraved bone ink and chromatic annotation bands
...
- Semantic state is consistent: vermilion fails, cobalt runs, ochre waits, mint completes, violet pauses
- No web fonts; no filled status pills; every color has a hierarchy, category, interaction, or state role

## Colors

A chromatic blueprint: two families of structure (ground and rule), a five-step bone ink ramp, three state inks, and three supporting annotation inks used for hierarchy and categorical distinction.
...
- **Vermilion Bright** (`{colors.mark-bright}`): The same pencil at text weight. Every vermilion _word_ — status text, gate names, error text, the "now" annotation — uses this step, for contrast reasons recorded below.
...
- **Ochre Hold** (`{colors.hold}` / `{colors.hold-bright}`): Awaiting attention, repeated, or pending. The bright step carries awaiting voices and the italic _da capo_ mark on a fix iteration.
- **Mint Complete** (`{colors.mint}` / `{colors.mint-bright}`): Completed and affirmative. It distinguishes successful voices from inert ones and gives primary affirmative actions a clear target.
- **Violet Pause** (`{colors.violet}` / `{colors.violet-bright}`): Paused state, knowledge surfaces, and the reflective annotation register.
- **Aqua Information** (`{colors.aqua}` / `{colors.aqua-bright}`): Informational navigation, data categories, and links that should not be confused with a currently running cue.
...
- **Ground Raised** (`{colors.ground-raised}`): The programme band and input surface. Shallow accent mixes create hover and current-row washes without introducing card elevation.
...
**The Annotation Ink Rule.** Vermilion means rejection, failure, or destructive action; cobalt means in progress; ochre means waiting; mint means successful or affirmative; violet means paused or reflective; aqua means informational. Section and navigation accents may cycle through the family for wayfinding, but they stay in shallow tints and never replace text, shape, or position.

**The 4.23 Rule.** `{colors.mark}` measures 4.23:1 on the blueprint ground, which is under the bar for 12–14px text. It is therefore permitted only where it is _not_ text — barlines, rules, borders, the caesura edge and hatch, icon strokes at 16px+. Small vermilion text always takes `{colors.mark-bright}`.

**The Labeled Quantity Rule.** Quantities may use the annotation family to distinguish categories, but every row retains a readable label and value. Color never implies success or failure in a chart, and is never the only category code.
...
- **Micro** (mono, 400, 0.6875rem, 1.2, tabular): Engraver's small type — stage labels under noteheads, rehearsal marks, right-margin task metadata. It sits _under_ the content it labels and must never compete with it.
...
One centred column, max width 72rem (`max-w-6xl`), padded 20px at phone width and 32px from the `sm` breakpoint, 28px top. There is no sidebar and no card grid: the page is a stack of _systems_, each a `<section>` with 40px of space beneath it, opened by a rehearsal mark and a title whose hairline rule runs to the right edge to carry the eye across.
...
Form language is stroke and shallow surface, not floating container. Structure comes from 1px hairlines for staves and dividers, 1px–2px barlines, square field borders, a 2px stroke for the affirmative button, a 1px box for a rehearsal mark, and a left rule for a finding or error. Filled shapes are meaningful — a filled notehead, categorical quantity bar, affirmative action, or current-navigation wash.
...
- **Affirmative:** A filled mint rectangle with ground text and a 2px mint border. This makes safe forward motion unmistakable.
- **Hover:** Mint brightens while the ground text remains stable.
- **Destructive / rejection:** A transparent vermilion control with bright vermilion text; hover fills it in. It cannot be confused with approval.
- **Focus:** A 2px `{colors.cue-bright}` outline offset 2px, on every interactive element without exception.

### Inputs / Fields

- **Style:** A square `{colors.ground-raised}` surface with a 1px `{colors.rule-strong}` border, mono at 0.875rem, and placeholder in `{colors.ink-muted}`.
- **Focus:** The border and surface shift toward cobalt and the cobalt focus ring appears.
- **Select:** Same field surface, plus an engraved chevron drawn in `{colors.ink-label}` at the right edge with the native appearance removed.
- **Label:** Italic serif annotation above the field, never inside it.

### Navigation

Movements, not tabs. A wrapped row of mono 0.75rem links under the programme head, separated from it by a hairline. Each product area owns a stable annotation hue; hover receives a 9% wash and the current movement a 15% wash plus a one-pixel underline. The deepest matching route wins, so a nested route marks only itself.

### System (section)

The page's unit. A shallow accent-tinted header carries a rehearsal mark, italic serif title, hairline rule, and optional mono tabular count. Marks A–F cycle through the annotation family so adjacent systems are distinguishable before their titles are read. 40px below it, the next system begins.
...
A magnitude read along the stave: a mono label at fixed width, a 10px `{colors.ground-band}` track filled from the categorical annotation family, and a right-aligned tabular value. Label and value remain the primary code, with a 1.5% minimum fill so a nonzero value never disappears.
...
- **Do** keep the semantic family stable: vermilion fails or rejects, cobalt runs, ochre waits, mint completes or approves, violet pauses, and aqua informs.
...
- **Do** pair every categorical quantity color with a readable label and value (The Labeled Quantity Rule).
...
- **Don't** let chart color imply state, and never make color the only category cue.
