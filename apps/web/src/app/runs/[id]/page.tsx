...
const fetchDiffArtifactContent = unstable_cache(
  async (runId: string, artifactId: string): Promise<DiffArtifactContent> => {
    ...
    if (artifact.kind !== 'diff') throw new Error('The selected artifact is not a diff.');
    const data = readDiffArtifactContent(artifact.content);
    if (!data) throw new Error('The diff artifact content is unavailable or malformed.');
    return data;
  },
  ['run-diff-artifact'],
  { revalidate: 3600 },
...
async function fetchDiffArtifact(
  runId: string,
  artifactId: string,
): Promise<DiffArtifactResult> {
  try {
    return { data: await fetchDiffArtifactContent(runId, artifactId) };
  } catch (error) {
    return {
      data: null,
...
export default async function RunDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const diffResult: DiffArtifactResult = diffArtifact
