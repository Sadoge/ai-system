...
import { groupHunkLines, limitHunkLines } from '@/lib/diff-view-model';
...
function DiffRow({ line }: { line: DiffLine }) {
  ...
  const status =
    line.kind === 'addition' ? 'added' : line.kind === 'deletion' ? 'deleted' : '';
  return (
    ...
    <>
      ...
      <DiffViewer files={files}>
        {parsed.files.map((file) => {
          const limited = limitHunkLines(file.hunks);
          return (
            <div
              key={file.id}
              className="diff-code"
            ...
            >
              ...
              <DiffHunks hunks={limited.visible} />
              {limited.remainingLineCount > 0 && (
                <RevealLines count={limited.remainingLineCount} contextOnly={false}>
                  <DiffHunks hunks={limited.remaining} keyPrefix="remaining-" />
                </RevealLines>
              )}
