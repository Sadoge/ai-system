...
describe('sliceFileHunks', () => {
  it('caps lines across hunk boundaries and reports the remainder', () => {
    const hunks = [
      { header: '@@ -1,3 +1,3 @@', lines: [context(1), context(2), context(3)] },
      { header: '@@ -10,3 +10,3 @@', lines: [context(10), context(11), context(12)] },
    ] as DiffHunk[];

    const result = sliceFileHunks(hunks, 4);
